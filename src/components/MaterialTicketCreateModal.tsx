...
export const MaterialTicketCreateModal: React.FC<MaterialTicketCreateModalProps> = ({
  ...
  const applyQrString = (val: string) => {
    setRawQr(val);
    const parsed = parseMaterialQr(val, type as 'normal' | 'tip');
                ...
                <div className="flex items-center justify-end pt-0.5">
                  {isParsed ? (
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const parsed = parseMaterialQr(rawQr, type as 'normal' | 'tip');
                        if (onShowScanError) {
                          onShowScanError({
                            type: 'invalid_format',
                            ...
                            message:
                              parsed.errorReason ||
                              (detectedType === 'tip' ? 'Mã QR không đủ 3 trường (Mã vật tư, Lô).' : 'Mã QR không đủ 6 trường thông tin quy chuẩn của Liên Châu.'),
                            rawQr,
                            parsed,
                          });
                        }
